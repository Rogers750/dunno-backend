...
export default function JeansBrandsScreen() {
  ...
  const saveBrands = async (skip = false) => {
    try {
      ...
      const res = await fetch(`${API_URL}/user/profile`, {
        ...
        body: JSON.stringify({
          jeans_brands: skip ? [] : selected,
        }),
      });

      if (res.ok) {
        router.replace("/profile/shirtsbrands");
      }
    } catch (error) {
      console.error("Error saving jeans brands:", error);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.container}>
        {/* ✅ Progress header with logo + dots */}
        <OnboardingHeader step={4} totalSteps={10} />

        <Text style={styles.title}>Favorite Jeans Brands</Text>
        <Text style={styles.subtitle}>
          Which brands do you wear the most for jeans? {"\n"}
...
const styles = StyleSheet.create({
  ...
  container: {
    ...
    paddingTop: 40,
    maxWidth: 400, // ✅ feels mobile-like even on web
    alignSelf: "center",
    width: "100%",
  },
  ...
  option: {
    width: "47%", // keeps 2 per row nicely
    height: 80, // ✅ bigger box
    borderWidth: 1,
    borderColor: "#e5e7eb",
    borderRadius: 16, // ✅ slightly bigger rounding
    margin: "1.5%", // ✅ even spacing between boxes
